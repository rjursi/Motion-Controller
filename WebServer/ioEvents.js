...
ioEvents.prototype.ioEventHandler = function(playerMgr, lobbyMgr, roomMgr){
	...
	this.io.sockets.on('connection', function (socket){
		...
		socket.on('gameover', function(){
			console.info('gameover_disconnect!!!!');
			
			if(game_sockets[web_socketIdTemp]){
				
				console.log("Web : Game Over disconnected");
				...
				if(roomMgr.roomIndex[web_socketIdTemp]){
					...
					// 해당 소켓이 포함된 room 폭파
					roomMgr.destroy_gameover(web_socketIdTemp, lobbyMgr)
		...
		*/
		...
		socket.on('ad_ChatMessage', function(message){
			...
			console.info("chat signal receive..");
			if(controller_sockets[socket.id]){
		...
		*/
		...
		socket.on('ui_holdByHit', function(playerId){
