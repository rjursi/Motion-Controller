...
ioEvents.prototype.ioEventHandler = function(playerManager){
	...
	this.io.on('connection', function (socket){
	  ...
	  socket.on('ad_login', function(userId){
		...
		socket.emit('ui_createPlayer', player);
		  
		socket.broadcast.emit('ui_addOtherPlayer', player);
   ...
   // disconnect 부분 구현 안됨
    ...
    socket.on('disconnect', function(){
	  ...
	  socket.on('ad_pause', function(userId){
	  ...
	  socket.on('ad_logout', function(userId){

		
		  
		// 안드로이드 단에서 어플리 종료되었을 경우 위에서 logout 신호를 받아서 접속한 플레이어중 없어지도록 처리\
		 socket.emit('ui_removeOtherPlayer', player);
	  ...
	  socket.on('ad_stop', function(userId){
